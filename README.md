...
Сторонний антивирус — то же самое в его настройках (добавить папку в доверенные).

Полностью убрать детект можно лишь подписью кода (EV-сертификат, платно — снимает SmartScreen и часть AV, но **не** детект на сам `WinDivert.sys`) и/или отправкой ложного срабатывания вендору; при каждом обновлении ядра хеш меняется, поэтому исключение папки — стандартный и практически неизбежный шаг.

### Прочие заметки

- **SmartScreen** на неподписанном exe → «Подробнее» → «Выполнить в любом случае».
- Для раздачи собирайте **release** (debug-сборка ловится антивирусами агрессивнее).
- Права администратора нужны для драйвера и службы — это инвариант Windows.
- Если winws падает с кодом 1 «cannot access ipset file … `*-user.txt`» — недостающие пользовательские списки. Zaprust создаёт их сам перед запуском (`ensure_user_lists`), т.к. winws запускается напрямую, минуя `service.bat load_user_lists` из сборки Flowseal.
